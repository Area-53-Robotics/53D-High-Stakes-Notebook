...
#pro-con(
  ...
  cons: [
    - *High Jamming Frequency* - Since only one ring should be taken at a time, multiple rings could jam in our intake before the hooks are ready to collect more rings.
  ],
  notes: [
...
#pro-con(
  ...
  cons: [
    - *Complexity* - The mechanism's design can be complex due to required synchronization between the flex wheel conveyer and the hood placement.
    - *Weight* - The hood intake may add extra weight to the robot compared to other intakes, which could affect its overall performance, speed, and the robot's ability to elevate in the endgame.
  ],
  notes: [
...
#pro-con(
  name: "Hybrid",
  image: image("brainstorm/hybrid-intake.excalidraw.svg"),
  description: [
    This hybrid design leverages the strengths of both systems to improve efficiency and performance. The hook intake captures rings using its hooked arms, securing them as the robot intakes. The hooks can engage the rings from various angles, providing flexibility in collection. At the top of the intake, the hood pivots down to push the rings into the mobile goal.
  ],
  pros: [
  ...
  cons: [
    ...
    - *Bulkiness* - The combined design may require more space on the robot, which could limit other robot functionalities or components.
  ],
)